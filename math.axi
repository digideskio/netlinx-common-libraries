...
/**
 * Checks if a string value contains only a positive numeric value.
 *
 * @param	a		a string to check
 * @return			a boolean, true is a numeric
 */
...
/**
 * Checks if a string value contains only a signed numeric value.
 *
 * @param	a		a string to check
 * @return			a boolean, true is a numeric
 */
define_function char is_numeric_signed(char a[])
{
    stack_var integer i

    i = 1
    if (a[1] == '-' && length_string(a) > 1) {
    	i = 2
    }

	for (; i <= length_string(a); i++) {
		if (!(a[i] >= $30 && a[i] <= $39)) {
			return 0
		}
	}
...
/**
 * Map a value within a set of original bounds to its relative position within
		...
		float toMin, float toMax)
{
  ...
  // base64 encoder only supports text strings - no binary data support at this time, sorry
define_function integer base64_encode(char to_encode[], char encoded[], line_length)
{
	stack_var long i
	stack_var char work[3]
	stack_var char enc[5]
	stack_var integer llen

	encoded = ''

	if (length_string(to_encode) == 0) {
		return 0
	}
	...
	for (i = 1; i <= length_string(to_encode); i = i + 3) {
		work[1] = to_encode[i]
		switch (length_string(to_encode) - i) {
			case 0:
				work[2] = $00
				work[3] = $00
			case 1:
		...
		if (length_string(to_encode) - i == 1) {
			enc[4] = '='
		} else if (length_string(to_encode) - i == 0) {
			enc[3] = '='
			enc[4] = '='
		}

		if (llen && (i % llen == 0)) {
			encoded = "encoded, enc, $0d, $0a"
		} else {
			encoded = "encoded, enc"
		}
	}
  ...
  // base64 decoder only supports text strings - no binary data support at this time, sorry
define_function integer base64_decode(char to_decode[65532], char decoded[49419])
{
	stack_var long i
	stack_var integer j
